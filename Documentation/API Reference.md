...
Think of Dynamit dynamic tables as being to regular static database classes what .NET `Dictionary` objects are to regular .NET classes. In fact, they are nothing but Starcounter database classes that implement the `IDictionary<string, object>` interface.
